<?php
 ...
 if ($s->o_server->id_owner != $lm->o_login->id) {
   $content = new Template("./tpl/error.tpl");
   $error = "You have no rights to view this patch level!";
   $content->set("error", $error);
   goto screen;
 }
 ...
 header("Content-Disposition: filename=plevel_".$s->o_server->name."-patchdiag.xref");
 $pd_head = "## PATCHDIAG TOOL CROSS-REFERENCE FILE AS OF ".date('M/d/y',time())." ##\n";
 $pd_head .= "##\n";
 $pd_head .= "## Generated by WeSunSolve! (http://wesunsolve.net)\n";
 $pd_head .= "## for server named: ".$s->o_server->name."\n";
 $pd_head .= "## and patch level: ".$s->name."\n";
 $pd_head .= "##\n";
